...
class SlashCommandListener(
  ...
  override def onSlashCommandInteraction(event: SlashCommandInteractionEvent): Unit = {
    event.getName() match {
      ...
      case "verify" =>
        handleVerifyCommand(event, VerifiedDB)

      case "submit-vc" =>
        val (updatedVerifiedDB, _) = handleSubmitVcCommand(event, VerifiedDB)
        VerifiedDB = updatedVerifiedDB

      case "verify-status" =>
        handleVerifyStatusCommand(event, VerifiedDB)
