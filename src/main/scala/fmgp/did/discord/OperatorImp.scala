...
object OperatorImp {
  type Services = Resolver & Agent &
    Operations // & UserAccountRepo & MessageItemRepo & Ref[MediatorTransportManager] & TransportDIDComm[Any]

  val protocolHandlerLayer: ULayer[
    ProtocolExecuter[Services, FailToParse]
  ] =
    ZLayer.succeed(
